...
nba_season_wins <- function(table, elements, first_season, last_season){
...
table1 <- table1[SeasonID >= as.numeric(paste(2, first_season, sep = "")) & SeasonID <= as.numeric(paste(2, last_season, sep = ""))][
    ,.(TeamName, WINS)][
      ,year := rep(seq(first_season, last_season), each = length(unique(table1$TeamName)))][
        , rank := frank(-WINS, ties.method = "random"), by = "year"][
          , value_rel := WINS/WINS[rank==1], by = "year"]
...
gg <- ggplot(table1, aes(rank, group = TeamName, fill = as.factor(TeamName),
  ...
  geom_text(aes(y = WINS, label = paste0(" ",round(WINS))), hjust = 0, size = 7) +
...
anim <- gg + transition_states(year, transition_length = 4, state_length = 1) +
  ...
  labs(title = paste("Wins by teams in seasons", 
...
animate(anim, (last_season - first_season + 1) * (length(unique(table1$TeamName)) + 20),
        fps = 20,  width = 1200, height = 1000, 
        renderer = gifski_renderer(paste(elements[1], "wins.gif", sep = "_")))
}

nba_season_wins(standings, "NBA", 2004, 2018)
